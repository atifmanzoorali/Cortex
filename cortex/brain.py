...
def natural_language_to_sql(question: str) -> str:
    ...
    if (
        ...
        and revenue_threshold
        and revenue_threshold >= 10000
    ):
        return f"""
...
def format_results(results, columns):
    ...
    for i, row in enumerate(results, 1):
        ...
        revenue = (
            row[col_idx.get("revenue_amount", -1)]
            if "revenue_amount" in col_idx
            else None
        )
        freq = (
            row[col_idx.get("revenue_frequency", -1)]
            if "revenue_frequency" in col_idx
            else ""
        )
        if revenue:
