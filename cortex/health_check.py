...
class HealthChecker:
    ...
    def _compare_nodes(
        self, node1: tuple, node2: tuple, col_idx: dict
        ...
        if rev1 and rev2 and abs(rev1 - rev2) / max(rev1, rev2) > 0.1:
            conflict = {
                ...
                "value2": rev2,
                "timestamp1": (
                    node1[col_idx.get("timestamp", 9)]
                    if col_idx.get("timestamp", 9) < len(node1)
                    else None
                ),
                "timestamp2": (
                    node2[col_idx.get("timestamp", 9)]
                    if col_idx.get("timestamp", 9) < len(node2)
                    else None
                ),
            }
        ...
        # Compare tech stacks
        tech1 = (
            json.loads(node1[col_idx.get("tech_stack", 4)])
            if node1[col_idx.get("tech_stack", 4)]
            else []
        )
        tech2 = (
            json.loads(node2[col_idx.get("tech_stack", 4)])
            if node2[col_idx.get("tech_stack", 4)]
            else []
        )
        ...
        if ts1 and ts2:
            ...
            try:
                ...
                if (date2 - date1).days > 365:
                    self._heal_conflict(
                        conflict,
                        newer_node_id=conflict["node2_id"],
                    ...
                    self._heal_conflict(
                        conflict,
                        newer_node_id=conflict["node1_id"],
    ...
    def _heal_conflict(self, conflict: dict, newer_node_id: str, older_node_id: str):
        ...
        newer_value = (
            conflict.get("value2")
            if newer_node_id == conflict["node2_id"]
            else conflict.get("value1")
        )
        older_value = (
            conflict.get("value1")
            if newer_node_id == conflict["node2_id"]
            else conflict.get("value2")
        )
        ...
        print(
            f"  [Healed] {field}: {older_value} -> {newer_value} (timeline reasoning)"
        )
    ...
    def _print_report(self, report: dict):
        ...
        if report["details"]:
            ...
            for i, conflict in enumerate(report["details"][:5], 1):  # Show first 5
                ...
                print(
                    f"   Nodes: {conflict['node1_id'][:8]}... vs {conflict['node2_id'][:8]}..."
                )

        if report["auto_healed"] > 0:
            print(
                "\n[Done] "
                + str(report["auto_healed"])
                + " conflict(s) auto-healed using timeline reasoning"
            )
        ...
        if report["manual_review_needed"] > 0:
            print(
                "\n[Review] "
                + str(report["manual_review_needed"])
                + " conflict(s) need manual review"
            )
