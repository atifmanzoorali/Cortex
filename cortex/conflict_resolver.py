...
def detect_conflict(
    existing_row: tuple, new_node: KnowledgeNode
) -> Dict[str, Any]:
    """
    ...
    existing_timestamp = (
        existing_row[9] if len(existing_row) > 9 else None
    )  # timestamp column
    ...
    if set(old_tech) != set(new_node.tech_stack):
        ...
        if existing_timestamp and new_timestamp:
            if new_timestamp > existing_timestamp:
                conflicts["reasoning"] += (
                    "Tech stack evolved from "
                    + str(old_tech)
                    + " to "
                    + str(new_node.tech_stack)
                    + ". "
                )
...
def log_conflict(
    existing_row: tuple, new_node: "KnowledgeNode", conflict_info: Dict[str, Any]
) -> None:
...
def should_auto_heal(conflict_info: Dict[str, Any]) -> bool:
    """
    ...
    return len(conflict_info["fields"]) > 0 and "newer" in conflict_info[
        "reasoning"
    ].lower()
